...
function printProducts() { 
    //connect to database. this only on first load.
    if (!alreadyConnected) {
        connectToDB();
        alreadyConnected = true;
    }
    //get number of items in database
    connection.create.query("SELECT item_id, product_name, department_name, price FROM products", function (err, res) {
        if (err) throw err;
        ...
        .then(function (res) {
            ...
            connection.create.query("SELECT stock_quantity FROM products WHERE item_id=" + res.ID, function (err, response) {
                ...
                //determine if there is enough quantity for the requested ID
                    ...
                    // if there is enough, UPDATE the database with the new amount
